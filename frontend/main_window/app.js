...
// 消息去重：防止同一消息通过 HTTP 和 WebSocket 双通道重复处理
...
function connectWebSocket() {
    ...
    appLog('正在连接 WebSocket...');
    ws = new WebSocket(WS_URL);

    ws.onopen = () => {
        appLog('WebSocket 已连接');
    ...
    ws.onmessage = (event) => {
        ...
        try {
            ...
            appLogError('WebSocket 消息解析失败:', e);
            return;
        }

        appLog('WS 收到消息 type:', data.type);

        switch (data.type) {
            // ===== 浮窗截图结果 =====
            case 'screen_result':
                if (data.req_id && isDuplicateMessage(data.req_id)) {
                    appLog('WS screen_result 重复消息，跳过 req_id:', data.req_id);
                    return;
                }
                if (data.req_id) addSeenId(data.req_id);

                const reply = data.data?.reply || '分析完成~';
                addMessage('agent', reply, data.data?.tool_calls || []);
                setIsThinking(false);
                break;
            ...
            // ===== agent_reply 已废弃（HTTP 直返），仅做防御性日志 =====
            case 'agent_reply':
...
function populateModelSelector(activeModel) {
    ...
    const presets = [
        { label: 'DeepSeek V4 Flash', value: 'deepseek-v4-flash', url: 'https://api.deepseek.com' },
        { label: 'DeepSeek V4 Pro', value: 'deepseek-v4-pro', url: 'https://api.deepseek.com' },
...
document.addEventListener('click', (e) => {
    ...
    if (e.target === document.getElementById('cliHubModal')) closeCliHub();
});

/* ==================== CLI-Hub 功能 ==================== */

let cliHubAllEntries = [];
let cliHubInstalledEntries = [];
let cliHubCurrentTab = 'browse';
let cliHubSelectedCli = null;

async function openCliHub() {
    document.getElementById('cliHubModal').classList.add('show');
    await loadCliHubCategories();
    await refreshCliHub();
}
...
async function loadCliHubCategories() {
    try {
        const res = await fetch(`${API_BASE}/cli-hub/categories`);
        const data = await res.json();
        const sel = document.getElementById('cliHubCategory');
        ...
        appLogError('加载 CLI-Hub 分类失败:', e.message);
    }
}

async function refreshCliHub() {
    const listEl = document.getElementById('cliHubList');
    const installedEl = document.getElementById('cliHubInstalledList');
    listEl.innerHTML = '<div class="cli-hub-loading">加载中...</div>';
    installedEl.innerHTML = '<div class="cli-hub-loading">加载中...</div>';

    try {
        const [listRes, installedRes] = await Promise.all([
            fetch(`${API_BASE}/cli-hub/list`),
            fetch(`${API_BASE}/cli-hub/installed`)
        ]);
        const listData = await listRes.json();
        const installedData = await installedRes.json();

        cliHubAllEntries = listData.entries || [];
        cliHubInstalledEntries = installedData.installed || [];

        renderCliHubList();
        renderCliHubInstalled();
    } catch (e) {
        appLogError('刷新 CLI-Hub 失败:', e.message);
        listEl.innerHTML = '<div class="cli-hub-loading">加载失败，请重试</div>';
    }
}

function filterCliHub() {
    renderCliHubList();
}

function renderCliHubList() {
    const listEl = document.getElementById('cliHubList');
    const searchTerm = (document.getElementById('cliHubSearch')?.value || '').toLowerCase();
    const category = document.getElementById('cliHubCategory')?.value || '';
    const source = document.getElementById('cliHubSource')?.value || 'all';

    let filtered = cliHubAllEntries;

    if (searchTerm) {
        filtered = filtered.filter(e =>
            e.name.toLowerCase().includes(searchTerm) ||
            (e.display_name || '').toLowerCase().includes(searchTerm) ||
    ...
    if (source && source !== 'all') {
        filtered = filtered.filter(e => e._source === source);
    }

    if (filtered.length === 0) {
        listEl.innerHTML = '<div class="cli-hub-empty">没有找到匹配的 CLI 工具</div>';
        return;
    }

    const installedNames = new Set(cliHubInstalledEntries.map(i => i.name));
    listEl.innerHTML = filtered.map(cli => {
        const isInstalled = installedNames.has(cli.name);
        return `
            <div class="cli-hub-card" onclick="showCliHubDetail('${cli.name}')">
                <div class="cli-hub-card-header">
                    <span class="cli-hub-card-icon">${cli.icon || '🔧'}</span>
                    <div class="cli-hub-card-info">
                        <span class="cli-hub-card-name">${escapeHtml(cli.display_name || cli.name)}</span>
                        <span class="cli-hub-card-version">v${cli.version || '1.0.0'}</span>
                    </div>
                    ${isInstalled ? '<span class="cli-hub-card-badge installed">已安装</span>' : ''}
                </div>
                <div class="cli-hub-card-desc">${escapeHtml(cli.description || '暂无描述')}</div>
                <div class="cli-hub-card-footer">
                    <span class="cli-hub-card-cat">${escapeHtml(cli.category || 'other')}</span>
                    <span class="cli-hub-card-source">${cli._source === 'harness' ? 'CLI-Anything' : '第三方'}</span>
                </div>
            </div>
        `;
    }).join('');
...
function renderCliHubInstalled() {
    const listEl = document.getElementById('cliHubInstalledList');
    if (cliHubInstalledEntries.length === 0) {
        listEl.innerHTML = '<div class="cli-hub-empty">暂无已安装的 CLI 工具</div>';
        return;
    }

    listEl.innerHTML = cliHubInstalledEntries.map(cli => `
        <div class="cli-hub-card" onclick="showCliHubDetail('${cli.name}')">
            <div class="cli-hub-card-header">
                <span class="cli-hub-card-icon">📦</span>
                <div class="cli-hub-card-info">
                    <span class="cli-hub-card-name">${escapeHtml(cli.display_name || cli.name)}</span>
                    <span class="cli-hub-card-version">v${cli.version || '1.0.0'}</span>
                </div>
                <span class="cli-hub-card-badge installed">已安装</span>
            </div>
            <div class="cli-hub-card-desc">安装于: ${cli.installed_at || '未知'}</div>
            <div class="cli-hub-card-footer">
                <span class="cli-hub-card-cat">${cli.install_strategy || '自动'}</span>
                <span class="cli-hub-card-source">${cli.source || '未知'}</span>
            </div>
        </div>
    `).join('');
}

function switchCliHubTab(tab) {
    cliHubCurrentTab = tab;
    document.querySelectorAll('.cli-hub-tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll(`.cli-hub-tab`).forEach(t => {
        const tabMap = { browse: '浏览', installed: '已安装', guide: '教程' };
        if (t.textContent === tabMap[tab]) t.classList.add('active');
    });
    document.getElementById('cliHubList').style.display = tab === 'browse' ? '' : 'none';
    document.getElementById('cliHubInstalledList').style.display = tab === 'installed' ? '' : 'none';
    document.getElementById('cliHubDetail').style.display = 'none';
    document.getElementById('cliHubGuide').style.display = tab === 'guide' ? '' : 'none';
...
function showCliHubDetail(name) {
    cliHubSelectedCli = cliHubAllEntries.find(e => e.name === name)
        || cliHubInstalledEntries.find(e => e.name === name);

    if (!cliHubSelectedCli) {
        // 可能来自已安装列表，需要从 allEntries 中补充
        const installed = cliHubInstalledEntries.find(e => e.name === name);
        if (installed) {
            cliHubSelectedCli = installed;
        } else {
            return;
        }
    }

    const isInstalled = cliHubInstalledEntries.some(i => i.name === name);

    document.getElementById('cliHubList').style.display = 'none';
    document.getElementById('cliHubInstalledList').style.display = 'none';
    document.getElementById('cliHubDetail').style.display = '';

    document.getElementById('cliHubDetailName').textContent = cliHubSelectedCli.display_name || cliHubSelectedCli.name;
    document.getElementById('cliHubDetailDesc').textContent = cliHubSelectedCli.description || '暂无描述';
    document.getElementById('cliHubDetailVersion').textContent = `v${cliHubSelectedCli.version || '1.0.0'}`;
    document.getElementById('cliHubDetailCategory').textContent = cliHubSelectedCli.category || 'other';
    document.getElementById('cliHubDetailSource').textContent = cliHubSelectedCli._source === 'harness' ? 'CLI-Anything' : '第三方';

    document.getElementById('cliHubInstallBtn').style.display = isInstalled ? 'none' : '';
    document.getElementById('cliHubUninstallBtn').style.display = isInstalled ? '' : 'none';
    document.getElementById('cliHubDetailStatus').textContent = '';
}

function closeCliHubDetail() {
    document.getElementById('cliHubDetail').style.display = 'none';
    if (cliHubCurrentTab === 'browse') {
        document.getElementById('cliHubList').style.display = '';
    } else {
        document.getElementById('cliHubInstalledList').style.display = '';
    }
}

async function loadCliHubGuide() {
    const guideEl = document.getElementById('cliHubGuide');
    guideEl.innerHTML = '<div class="cli-hub-guide-loading">加载教程中...</div>';

    try {
        const resp = await fetch(`${API_BASE}/cli-hub/guide`);
        const data = await resp.json();

        let html = '';
        // 快速入门
        if (data.quick_start) {
            html += `<div class="cli-hub-guide-quickstart">${marked.parse(data.quick_start)}</div>`;
        }
        // 章节
        if (data.sections) {
            data.sections.forEach((section, idx) => {
                html += `
                    <div class="cli-hub-guide-section">
                        <div class="cli-hub-guide-section-header" onclick="toggleGuideSection(${idx})">
                            <span class="cli-hub-guide-section-icon">${getGuideIcon(section.icon)}</span>
                            <span class="cli-hub-guide-section-title">${section.title}</span>
                            <span class="cli-hub-guide-section-toggle" id="guideToggle${idx}">▼</span>
                        </div>
                        <div class="cli-hub-guide-section-content" id="guideContent${idx}">
        ...
        guideEl.innerHTML = `<div class="cli-hub-guide-error">加载教程失败: ${err.message}</div>`;
    }
}

function toggleGuideSection(idx) {
    const content = document.getElementById('guideContent' + idx);
    const toggle = document.getElementById('guideToggle' + idx);
...
function getGuideIcon(icon) {
    const icons = {
        house: '🏠', play: '▶️', download: '📥', robot: '🤖',
        grid: '📊', question: '❓', book: '📖', star: '⭐',
        wrench: '🔧', terminal: '💻', code: '📝'
    };
    return icons[icon] || '📌';
}

async function installCliHub() {
    if (!cliHubSelectedCli) return;

    const statusEl = document.getElementById('cliHubDetailStatus');
    const btn = document.getElementById('cliHubInstallBtn');
    statusEl.textContent = '⏳ 正在安装...';
    statusEl.style.color = 'var(--text-secondary)';
    btn.disabled = true;

    try {
        const res = await fetch(`${API_BASE}/cli-hub/install`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: cliHubSelectedCli.name })
        });
        const data = await res.json();

        if (data.success) {
            statusEl.textContent = '✅ ' + (data.result || '安装成功');
            statusEl.style.color = '#22c55e';
            await refreshCliHub();
            showCliHubDetail(cliHubSelectedCli.name);
        } else {
            statusEl.textContent = '❌ ' + (data.error || '安装失败');
...
async function uninstallCliHub() {
    if (!cliHubSelectedCli) return;

    const statusEl = document.getElementById('cliHubDetailStatus');
    const btn = document.getElementById('cliHubUninstallBtn');
    statusEl.textContent = '⏳ 正在卸载...';
    statusEl.style.color = 'var(--text-secondary)';
    btn.disabled = true;

    try {
        const res = await fetch(`${API_BASE}/cli-hub/uninstall`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: cliHubSelectedCli.name })
        });
        ...
        if (data.success) {
            statusEl.textContent = '✅ ' + (data.result || '卸载成功');
            statusEl.style.color = '#22c55e';
            await refreshCliHub();
            closeCliHubDetail();
        } else {
            statusEl.textContent = '❌ ' + (data.error || '卸载失败');
            statusEl.style.color = '#ef4444';
        }
    } catch (e) {
        statusEl.textContent = '❌ 网络错误: ' + e.message;
