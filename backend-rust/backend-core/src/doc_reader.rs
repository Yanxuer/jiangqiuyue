...
pub fn delete_recent_path(memory_path: &Path, path_str: &str) -> Vec<RecentPath> {
    log::info!(
        "[docs/delete-path] 开始删除最近路径: path_str={}, memory_path={}",
        path_str,
        memory_path.display()
    );

    log::debug!("[docs/delete-path] 正在加载最近路径列表...");
    let mut paths = load_recent_paths(memory_path);
    log::info!(
        "[docs/delete-path] 加载完成，当前共 {} 条最近路径",
        paths.len()
    );
    for (i, p) in paths.iter().enumerate() {
        log::debug!(
            "[docs/delete-path]   [{}] path={}, name={}, time={}",
            i, p.path, p.name, p.time
        );
    }

    let target = std::fs::canonicalize(path_str)
        .map(|p| p.display().to_string())
        .unwrap_or_else(|_| path_str.to_string());
    log::info!("[docs/delete-path] 规范化后的目标路径: {}", target);

    let before_count = paths.len();
    paths.retain(|p| {
        let keep = std::fs::canonicalize(&p.path)
            .map(|cp| cp.display().to_string() != target)
            .unwrap_or(true);
        if !keep {
            log::info!("[docs/delete-path] 匹配到待删除路径: {}", p.path);
        }
        keep
    });
    ...
    log::info!(
        "[docs/delete-path] 过滤完成，删除了 {} 条，剩余 {} 条",
        removed,
        paths.len()
    );
    ...
    if removed == 0 {
        log::warn!(
            "[docs/delete-path] 未找到匹配的路径可删除，target={}",
            target
        );
    }

    log::debug!("[docs/delete-path] 正在保存更新后的最近路径列表...");
    save_recent_paths(memory_path, &paths);
    log::info!(
        "[docs/delete-path] 保存完成，文件路径: {}",
        memory_path.join("recent_paths.json").display()
    );
